...
export class TitleBar {
    ...
    private initializeTitleBar = (isShareNeeded: Boolean): void => {
        ...
        // tslint:disable-next-line:max-line-length
        const btnStyles: string = 'float:right;background: transparent;box-shadow:none; font-family: inherit;border-color: transparent;'
    ...
    private wireEvents = (): void => {
        ...
        this.open.element.addEventListener('click', (e: Event) => {
            if ((e.target as HTMLInputElement).id === 'de-open') {
                const fileUpload: HTMLInputElement = document.getElementById('uploadfileButton') as HTMLInputElement;
                fileUpload.value = '';
                fileUpload.click();
            }
        });
    ...
    private addButton(iconClass: string, btnText: string, styles: string, id: string, tooltipText: string, isDropDown: boolean, items?: ItemModel[]): Button | DropDownButton {
        ...
        if (isDropDown) {
            // tslint:disable-next-line:max-line-length
            const dropButton: DropDownButton = new DropDownButton({ select: this.onExportClick, items, iconCss: iconClass, cssClass: 'e-caret-hide', content: btnText, open: (): void => { this.setTooltipForPopup(); } }, button);
            return dropButton;
        } else {
